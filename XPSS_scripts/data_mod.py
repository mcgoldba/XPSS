...
#            min_elev_sort.append(pipe_min_elev[sort_lst[i]])
    ...
    def get_pipe_elev_data(self, Pipe_props, Node_props, sort_lst, node_srt_lst, In, res_elev):
        ...
        In = np.ma.masked_equal(In, 0, copy=False)  # replace all 0 values with NaN
        
        Min_elevation = (In.T*Elevation).T
        ...
        Static_head = -(Min_elevation - Elevation[0][0])
        ...
        Pipe_props = self.append_col_to_table(Pipe_props, Min_elevation, 'Minimum Elevation [ft]')
        Pipe_props = self.append_col_to_table(Pipe_props, Static_head, 'Static Head [ft]')
