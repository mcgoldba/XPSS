...
#                return f_
        ...
        Pipe_props = self.append_col_to_table(Pipe_props, Accum_loss, ['Accumulated Friction Loss [ft]'], sort=sort_lst_inv)
        
        Node_props = self.append_col_to_table(Node_props, P, ['Pressure [ft]'], data_type='float', sort=node_srt_lst_inv)
    ...
    def convert_length(self, Pipe_props, native_units):
        """
        Converts the pipe length that are import in the map CRS units to the native XPSS units.
        ...
        PSSDataMod().log_progress("Map units are: "+str(map_units))
        PSSDataMod().log_progress("XPSS units are set to: "+str(native_units))
        ...
        if map_units is not native_units:
            ...
            if map_units == 0:  #map units = meters
                if native_units == 1:  #native units = feet
